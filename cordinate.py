...
x, y = lcc.convert(lon=lon, lat=lat, x=None, y=None, code=0)
print(f"(lon, lat) -> (x, y): ({lon}, {lat}) -> ({x}, {y})")

# 격자 -> 위경도 변환
# lon, lat = lcc.convert(lon=None, lat=None, x=x, y=y, code=1)
# print(f"(x, y) -> (lon, lat): ({x}, {y}) -> ({lon}, {lat})")